...
def single_task_trial(search_space: dict, rand_seed=42):
    ...
    exp_name = 'train.graph.' + parameter_dict['model_type'] + '.bs' + str(parameter_dict['per_gpu_train_batch_size']) + \
               '.lr' + str(parameter_dict['learning_rate']) +'.lrs' + str(parameter_dict['learning_rate_schema']) + '.data' +str(parameter_dict['daug_type']) + '.seed' +str(rand_seed)
...
def HypeParameterSpace():
    learning_rate = {'name': 'learning_rate', 'type': 'choice', 'values': [2e-6, 5e-6, 1e-5]}
    ...
    gnn_drop = {'name': 'gnn_drop', 'type': 'choice', 'values': [0.25, 0.3]} #0.3
    gnn_attn_drop = {'name': 'gnn_attn_drop', 'type': 'choice', 'values': [0.25, 0.3]}  # 0.3
    ...
    daug_type = {'name': 'daug_type', 'type': 'choice', 'values': ['hgn_low_sae']}
