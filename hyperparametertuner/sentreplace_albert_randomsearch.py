...
def HypeParameterSpace():
    ...
    sent_lambda = {'name': 'sent_lambda', 'type': 'choice', 'values': [5, 8]} ##
    ...
    sent_replace_ratio = {'name': 'sent_replace_ratio', 'type': 'choice', 'values': [0.1, 0.25]}
    ...
    search_space = [learning_rate, per_gpu_train_batch_size, gradient_accumulation_steps, sent_lambda, layer_wise_lr_decay,
                    lr_scheduler, fine_tuned_encoder, daug_type, devf_type,  optimizer, trans_drop, num_train_epochs, replace_prob,
                    model_type, encoder_name_or_path, transformer_hidden_dim, transformer_head_num, sent_replace_ratio]
...
def generate_random_search_bash(task_num, seed=42):
    ...
    for i in range(task_num):
        ...
        config_json_file_name = 'train.sent_replace.' + rand_hype_dict['model_type'] + '.data.' + rand_hype_dict['daug_type'] + 'sdr.' + str(rand_hype_dict['sent_replace_ratio']) \
        ...
        with open(jobs_path + 'jd_hotpotqa_' + config_json_file_name +'.sh', 'w') as rsh_i:
            ...
            command_i = "CUDA_VISIBLE_DEVICES=0,1,2,3 python -m torch.distributed.launch --nproc_per_node=4 jdsrhotpotqatrain.py --config_file " + \
                        json_file_path + config_json_file_name
