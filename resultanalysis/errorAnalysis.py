        ...
        if len(pred_idxes) > 1:
            ...
            if question_type == 'bridge':
                for i in range(len(pred_idxes), mask_sum_num):
                    ...
                    if prune_scores[s_idx] >= g_score * 0.5:
...
def prediction_score_analysis_adaptive_threshold(raw_data, predictions, prediction_scores, at_thresholds):

    def positive_neg_score(scores, mask, names, gold_names, threshold_score, pred_names):
        assert len(scores) == len(mask)
        mask_sum_num = int(sum(mask))
        prune_names = names[:mask_sum_num]
        gold_name_set = set(gold_names)
    ...
    for row in raw_data:
        ...
        max_n_metrics = update_sp(prediction=max_n_names, gold=sp_golds)
        threshold_metric_dict['max_n'].append((ans_metrics, max_n_metrics))
        threshold_metrics = update_sp(prediction=threshold_names, gold=sp_golds)
        threshold_metric_dict['threshold_n'].append((ans_metrics, threshold_metrics))
