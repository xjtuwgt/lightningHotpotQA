...
#        python scripts/4_multihop_ps.py $INPUT_FILE $OUTPUT_PROCESSED/doc_link_ner.json $OUTPUT_PROCESSED/ner.json $OUTPUT_PROCESSED/para_ranking.json $OUTPUT_PROCESSED/multihop_para.json
        ...
        python jdscripts/5_unifiedqa_dump_features.py --para_path $OUTPUT_PROCESSED/multihop_para.json --full_data $INPUT_FILE --model_name_or_path $ALLENAI_ROOT/unifiedqa-t5-large --ner_path $OUTPUT_PROCESSED/ner.json --model_type unifiedqa --tokenizer_name $ALLENAI_ROOT/unifiedqa-t5-large --output_dir $OUTPUT_FEAT --doc_link_ner $OUTPUT_PROCESSED/doc_link_ner.json
