...
#        python dataugmentation/reverse_para_order.py $INPUT_FILE $OUTPUT_PROCESSED/multihop_para.json $OUTPUT_PROCESSED/reverse_multihop_para.json
        ...
        python jdscripts/5_ext_graph_dump_features.py --para_path $OUTPUT_PROCESSED/multihop_para.json --full_data $INPUT_FILE --model_name_or_path roberta-large --ner_path $OUTPUT_PROCESSED/ner.json --model_type roberta --tokenizer_name roberta-large --output_dir $OUTPUT_FEAT --doc_link_ner $OUTPUT_PROCESSED/doc_link_ner.json --ranker hgn --data_type $DATA_TYPE
