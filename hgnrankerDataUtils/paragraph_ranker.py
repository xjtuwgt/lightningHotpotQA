...
def para_ranker_model(args, encoder, model, dataloader, example_dict, topk=2, gold_file=None):
    ...
    for batch in tqdm(dataloader):
        ...
        for i in range(predict_support_para_np.shape[0]):
            ...
            rank_paras_dict[cur_id] = []
            rank_paras_dict[cur_id].extend(title_score_pair_list)
            ##+++++++++
...
def albert_para_ranker_model(args, encoder, model, dataloader, example_dict, topk=2, gold_file=None):
    #### model_type, device
    encoder.eval()
    model.eval()
    ##++++++
