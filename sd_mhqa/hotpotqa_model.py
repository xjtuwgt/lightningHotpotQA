...
def para_sent_state_feature_extractor(batch, input_state: Tensor):
    ...
    assert (sent_start < input_state.shape[1]).sum() == input_state.shape[1] \
           and (sent_end < input_state.shape[1]).sum() == input_state.shape[1], '{}\t{}\t{}'.format(sent_start, sent_end, input_state.shape[1])
