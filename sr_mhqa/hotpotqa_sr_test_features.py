...
from sr_mhqa.hotpota_sr_data_loader import HotpotTestDataset, HotpotDataset

from model_envs import MODEL_CLASSES
from sr_mhqa.hotpotqa_sr_dump_examples import get_cached_filename
from sd_mhqa.hotpotqaUtils import json_loader
from sr_mhqa.hotpotqa_sr_data_structure import Example
from sd_mhqa.hotpotqaUtils import case_to_features, trim_input_span
from sr_mhqa.hotpotqa_sr_utils import example_sent_replacement
...
def sent_repalce_case_to_feature_checker(para_rank_file: str,
    ...
    for row in tqdm(full_data):
        ...
        replace_example_i, replace_ids = example_sent_replacement(case=example_i, replace_ratio=0.25)
        print(replace_ids)
...
def trim_case_to_feature_checker(para_rank_file: str,
    ...
    for row in tqdm(full_data):
        ...
        for idx, trim_ans_span in enumerate(ans_spans):
            decoded_ans = tokenizer.decode(doc_input_ids[trim_ans_span[0]:(trim_ans_span[1] + 1)])
        ...
        for idx, trim_ans_span in enumerate(trim_ans_spans):
            decoded_ans = tokenizer.decode(trim_doc_input_ids[trim_ans_span[0]:(trim_ans_span[1] + 1)])
            print('{}\t{}'.format(idx+1, decoded_ans))
        print('*' * 75)
