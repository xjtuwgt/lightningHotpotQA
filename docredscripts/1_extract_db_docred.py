...
for data in tqdm(input_data):
    ...
    for title in context.keys():
        ...
        if title not in title_to_id:
            continue
        sent_cnt += len(context[orig_title])
    ...
    for title in context.keys():
        ...
        if title not in title_to_id:
            ...
            db_context = pickle.loads(doc_db.get_doc_text(doc_id)) # list of string,type: bytes
            if len(db_context) != sent_cnt:
                print(type(db_context), len(db_context), sent_cnt)
                mismatch_context.append((db_context, context))
                continue
            ...
            # title = title + idx
            # print("after search in db ", title)
