...
def extract_context_ner(full_data, ner_data=None):
    ...
    for case in full_data:
        ...
        sent_offset = 0
        for orig_title, sents in case['context']:
            context_ner = []
            title = orig_title[:-2]
            curt_ner_data = ner_data[title]['text_ner'][sent_offset:len(sents)+sent_offset]
            for sent, sent_ner in zip(sents, curt_ner_data):
                context_ner.append([])
                for ner in sent_ner:
            ...
            context_guid2ner[guid].append([orig_title, context_ner])
